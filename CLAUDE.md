...
Sections §1–§24 substantially complete (§18 partial — MUL/MULL m-parameter cycles done, LDM/STM/STR overlap deferred). Current head implements a working 3-stage Fetch/Decode/Execute pipelined ARM7TDMI-S core (`rtl/core/arm7tdmis_core_pipelined.sv`) with: full ARM + Thumb decode coverage; branch/BL/BX; all addressing modes for LDR/STR/LDM/STM/SWP including LDM ^ user-bank (with and without PC-in-list / CPSR-from-SPSR); MUL/MLA/UMULL/SMULL with m-parameter cycle accuracy; ARM↔Thumb interworking; exception entry for SWI/UNDEF/IRQ/FIQ/PABT/DABT with ABORT sampling and writeback suppression; coprocessor handshake outputs (CPnMREQ/CPSEQ/CPnTRANS/CPnOPC/CPTBIT/CPnI) with UNDEF trap for unaccepted instructions; CP14 DCC data transfer (MCR/MRC p14, c0); CP15 Main ID Register read; JTAG TAP with IDCODE, scan chain 2 (ICE-RT register R/W), scan chain 1 (instruction-injection TAP-side latch), and RESTART; EmbeddedICE-RT register bank with WP0/WP1 comparators (XNOR+mask), CHAIN/RANGE coupling between WP1→WP0, Vector Catch register, Debug Status Register, debug-state FSM (HALTED + RESTART exit), DBGACK forced via control[0] or by FSM, IFEN gating, 2-flop DBGRQ/DBGBREAK synchronizers; ETM-facing DBGnEXEC / DBGINSTRVALID. Smoke + UMULL integration test + 10 unit tests pass.

What's left: cycle accuracy for LDR/STR/LDM/STM (requires address-phase / data-phase overlap on the pipelined bus, which is a substantial restructuring); UMLAL/SMLAL accumulate forms (need 4 regfile read ports or a 2-cycle accumulator read); scan-chain-1 core injection path (needs CLKEN coordination across halt/unhalt windows); §25 scan/test wrapper and §26 Cyclone V SDC + part selection (FPGA bring-up milestone); §27 broader conformance test expansion; §28-§29 milestone checklists.
