...
def install_overlay(framework_root: pathlib.Path) -> dict[str, str]:
    ...
    qsf_source = framework_root / "Template.qsf"
    qsf_text = qsf_source.read_text(encoding="utf-8")
    old_seed = "set_global_assignment -name SEED 1"
    new_seed = (
        f"set_global_assignment -name SEED {EXPECTED_FITTER_SEED}"
    )
    if qsf_text.count(old_seed) != 1:
        raise ValueError("official template fitter seed marker changed")
    qsf_source.write_text(
        qsf_text.replace(old_seed, new_seed),
        encoding="utf-8",
    )
    qip_lines = [
...
def parse_reports(output_dir: pathlib.Path) -> tuple[dict[str, Any], list[str]]:
    ...
    seed_match = re.search(
        r"^\s*;\s*Fitter Initial Placement Seed\s*;\s*(\d+)\s*;",
        fit_report,
        re.MULTILINE,
    )
    fitter_seed = int(seed_match.group(1)) if seed_match else None
    if fitter_seed != EXPECTED_FITTER_SEED:
        errors.append(f"unexpected fitter seed: {fitter_seed!r}")
...
def validate_evidence(
    ...
    *,
    ...
    if (
        ...
        or result.get("fitter_seed") != EXPECTED_FITTER_SEED
