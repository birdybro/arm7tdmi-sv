...
|---|---|
| ARM/Thumb architecture | **VERIFIED** for the directed requirements in §31.3, including all Thumb format families, reserved space, PC/PSR rules, operand policies, dependencies, and state changes. Independent QEMU differential, constrained-random, pinned public ARM/Thumb suite, compiler-program, and sanitizer/X-state soak gates also pass; cycle-cross, functional-coverage, and formal closure remain **PARTIAL** under §31.10. |
...
| Release evidence | **PARTIAL**. Regressions fail hard; an independent 77-retirement QEMU differential, 32 × 256 constrained-random campaign, pinned public suite and GCC ARM/Thumb/interworking execution, a 256-seed sanitizer/X-state soak, bidirectional requirement/source traceability, machine-readable reports, mutation tests, and a hashed evidence archive exist. Cycle-cross/functional/formal validation and FPGA release gates remain open. |
