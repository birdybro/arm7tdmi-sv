// UMLAL integration test: validates the §9d 2-cycle commit path
// end-to-end inside the pipelined core. Program at tb/programs/umlal_test.hex:
//
//   0x00: B 0x20
//   ...
//   0x20: MOV r0, #0x4   ; 2^28
//   0x24: MOV r1, #0x10         ; 16
//   0x28: UMLAL r2, r3, r0, r1  ; r2:r3 = r0 * r1 = 2^32
...
;
    ...
    localparam string INIT_HEX        = "../tb/programs/umlal_test.hex";
    ...
    task automatic check_reg(input int idx, input logic [31:0] expected,
        ...
        if (u_dut.u_core.u_regfile.regs[idx] !== expected) begin
            $display("[umlal] FAIL %s (r%0d): expected %08x got %08x",
    ...
    initial begin
        $dumpfile("umlal.fst");
        $display("[umlal] starting; CYCLE_LIMIT=%0d", CYCLE_LIMIT);
        wait (nRESET);
        repeat (CYCLE_LIMIT) @(posedge CLK);

        // UMLAL r2, r3, r0, r1 with seeds (r0=4, r1=0x10, r2=0x100, r3=1):
        //   product   = 4 * 16 = 0x40
        //   acc       = {r3=1, r2=0x100} = 0x1_00000100
        //   sum       = 0x1_00000140
        //   r2 (low)  = 0x00000140
        //   r3 (high) = 0x00000001
        check_reg(0, 32'h00000004, "r0 setup");
        check_reg(1, 32'h00000010, "r1 setup");
        check_reg(2, 32'h00000140, "r2=UMLAL low half (acc.lo + product.lo)");
        check_reg(3, 32'h00000001, "r3=UMLAL high half (unchanged, no carry)");

        if (u_dut.u_core.pc_q !== 32'h00000034) begin
            $display("[umlal] FAIL pc_q: expected 0x34 self-loop, got %08x",
                     u_dut.u_core.pc_q);
            errors = errors + 1;
        end

        if (errors != 0)
            $fatal(1, "[umlal] FAIL (%0d errors)", errors);
        $display("[umlal] PASS");
        $finish;
    end
    ...
    initial begin
        ...
        $fatal(1, "[umlal] TIMEOUT after %0d cycles", CYCLE_LIMIT + 32);
    end
