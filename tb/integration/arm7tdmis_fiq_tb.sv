...
// end-to-end inside the pipelined core. Program at tb/programs/fiq_test.hex:
//
//   0x00: B 0x20
//   ...
...
;
    ...
    localparam string INIT_HEX        = "../tb/programs/fiq_test.hex";
    ...
    task automatic check_reg(input int idx, input logic [31:0] expected,
        ...
        if (u_dut.u_core.u_regfile.regs[idx] !== expected) begin
            $display("[fiq] FAIL %s (r%0d): expected %08x got %08x",
    ...
    initial begin
        ...
        nFIQ = 1'b0;
        wait (u_dut.u_core.cpsr.m == 5'b10001);  // MODE_FIQ
        @(posedge CLK);
        nFIQ = 1'b1;
    end
    ...
    initial begin
        $dumpfile("irq.fst");
        $display("[fiq] starting; CYCLE_LIMIT=%0d", CYCLE_LIMIT);
        wait (nRESET);
        repeat (CYCLE_LIMIT) @(posedge CLK);

        // After ABORT entry:
        //   • cpsr.m = MODE_FIQ (5'b10001)
        ...
        if (u_dut.u_core.u_regfile.regs[0] === 32'h0) begin
            $display("[fiq] FAIL r0 == 0 — main code never ran before ABORT");
            errors = errors + 1;
        end

        check_reg(6, 32'h00000034, "r6 = FIQ handler marker (MOV r6, #0x34)");

        if (u_dut.u_core.cpsr.m !== 5'b10001) begin
            $display("[fiq] FAIL cpsr.m: expected ABORT (10001), got %05b",
                     u_dut.u_core.cpsr.m);
            errors = errors + 1;
        end

        if (u_dut.u_core.pc_q !== 32'h00000044) begin
            $display("[fiq] FAIL pc_q: expected 0x44 (ABORT handler self-loop), got %08x",
                     u_dut.u_core.pc_q);
            errors = errors + 1;
        end

        if (errors != 0)
            $fatal(1, "[fiq] FAIL (%0d errors)", errors);
        $display("[fiq] PASS");
        $finish;
    end
    ...
    initial begin
        ...
        $fatal(1, "[fiq] TIMEOUT after %0d cycles", CYCLE_LIMIT + 32);
    end
