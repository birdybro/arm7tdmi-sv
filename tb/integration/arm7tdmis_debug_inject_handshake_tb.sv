...
//
// Enter halt through DBGRQ, inject a MOV and then a 12-register LDM through
// public scan chain 1, and stall CLKEN in the middle of the LDM. Each scan
// update must execute exactly one injected instruction, wait for its real
// completion boundary, and re-freeze in S_EXEC. Normal code queued at debug
// entry must never retire during either injection.
...
;
    ...
    initial begin : run_test
        ...
        // MOV r0,#0x100
        inject_debug_speed(32'hE3A0_0C01);
        ...
        if (!block_started)
            fail("LDM never reached S_BLOCK_DATA");

        // Stall in the middle of the external transfer and require every
        // address-class output to remain stable.
        @(negedge CLK);
        stalled_addr  = ADDR;
        stalled_wdata = WDATA;
        stalled_write = WRITE;
        ...
        repeat (7) begin
            ...
            if ({ADDR, WDATA, WRITE, SIZE, PROT, TRANS, LOCK}
                ...
                fail("bus outputs changed while CLKEN was low");
        end
        @(negedge CLK);
        CLKEN = 1'b1;
